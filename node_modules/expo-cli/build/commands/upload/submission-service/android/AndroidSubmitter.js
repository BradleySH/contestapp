...
function _curryRight() {
  const data = _interopRequireDefault(require("lodash/curryRight"));

  _curryRight = function () {
    return data;
  };
...
class AndroidSubmitter {
  ...
  async submitAsync() {
    ...
    if (this.ctx.mode === _types().SubmissionMode.online) {
      ...
      const submissionConfig = await AndroidOnlineSubmitter.formatSubmissionConfigAndPrintSummary(this.options, resolvedSourceOptions);
...
class AndroidOnlineSubmitter {
  ...
  async submitAsync() {
    ...
    try {
      submissionId = await _SubmissionService().default.startSubmissionAsync(_SubmissionService2().Platform.ANDROID, this.submissionConfig);
    ...
    try {
      while (!submissionCompleted) {
        ...
        submission = await _SubmissionService().default.getSubmissionAsync(submissionId);
...
const SummaryHumanReadableValues = {
  ...
  archivePath: (0, _curryRight().default)(breakWord)(50),
  archiveUrl: (0, _curryRight().default)(breakWord)(50)
};
